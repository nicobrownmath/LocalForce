...
class CategoryContainer {
    ...
    std::map<CatContainerKey, unsigned> keySizes;
    ...
    bool Add(const LifeState &startState, const LifeState &finalState, const std::vector<LifeState> &interimStates, const CatContainerKey &key) {
        if (categories.find(key) == categories.end()) {
            keys.push_back(key);
            keySizes[key] = 0;
        }
        if (keySizes[key] < maxOutputsPerRow)
        {
            keySizes[key]++;
            categories[key].push_back(startState);
            for (auto &state : interimStates) {
                categories[key].push_back(state);
            }
...
class Catalyst {
    ...
    bool spark = false;
    LifeState sparkStart;
    unsigned sparkGens = 0;
    ...
    static void AddCatalyst(std::vector<Catalyst> *catalysts, const std::vector<std::string> &elems) {
        ...
        catalyst.sparkStart = catalyst.state;
        ...
        while (index < (int)elems.size()) {
            ...
            else if (elems[index] == "spark") {
                catalyst.spark = true;
                catalyst.sacrificial = true;
                catalyst.sparkGens = stoi(elems[index + 1]);
                //update state
                for (unsigned gen = 0; gen < catalyst.sparkGens; gen++) {
                    catalyst.state.Step();
                }
                index += 2;
            }
    ...
    void Transform(SymmetryTransform transform) {
        ...
        sparkStart.Transform(transform);
...
class SearchData {
    ...
    std::vector<LifeState> past1NeighborWithPeriod;
    std::vector<LifeState> past2NeighborsWithPeriod;
    std::vector<LifeState> past3NeighborsWithPeriod;
    ...
    std::vector<LifeState> intermediateForms;
};
...
class Searcher {
    ...
    void IterativeSearch(const SearchData &searchData) {
        ...
        std::vector<std::vector<LifeState>> past1NeighborWithPeriodEvolution = {searchData.past1NeighborWithPeriod};
        std::vector<std::vector<LifeState>> past2NeighborsWithPeriodEvolution = {searchData.past2NeighborsWithPeriod};
        std::vector<std::vector<LifeState>> past3NeighborsWithPeriodEvolution = {searchData.past3NeighborsWithPeriod};
        ...
        for (unsigned generation = searchData.generation; generation < maxGenerationAllowed; generation++) {
            ...
            if (canIterate) { //these are only needed for catalyst placement
                ...
                past1NeighborWithPeriodEvolution.push_back(past1NeighborWithPeriodEvolution[generationIndex]);
                past2NeighborsWithPeriodEvolution.push_back(past2NeighborsWithPeriodEvolution[generationIndex]);
                past3NeighborsWithPeriodEvolution.push_back(past3NeighborsWithPeriodEvolution[generationIndex]);
                        ...
                        //only need to bother if there are few enough catalysts/symmetries
                            ...
                            for (int catID = searchData.catalysts.size() - 1; catID >= 0; catID--) {
                                ...
                                if (!catsRestored[catID] && !catalysts[searchData.catalysts[catID][0]].sacrificial) {
                                    ...
                                    filterState = CatalystCheckState(searchData.catalysts[catID][0], stateEvolution[modelGeneration], searchData.catalysts[catID][1], searchData.catalysts[catID][2], modelGeneration + searchData.generation).second;
        ...
        /*for (int catID = 0; catID < (int)searchData.catalysts.size(); catID++) {
            if (false && catalysts[searchData.catalysts[catID][0]].checkRecoveryAlways) {
                ...
                for (unsigned i = 0; (int)i < remainingSteps; i++) {
                    testState.Step();
                    std::pair<bool, LifeState> filtering = CatalystCheckState(searchData.catalysts[catID][0], testState, searchData.catalysts[catID][1], searchData.catalysts[catID][2], i + stateEvolution.size() + searchData.generation);
                    if (!filtering.first) {
                        break;
                    }
        ...
        if (searchData.numCatalysts >= params.minCatalysts && searchData.numTransparentCatalysts >= params.minTransparentCatalysts && searchData.numSacrificialCatalysts >= params.minSacrificialCatalysts) {
            if (!searchData.supressOutput && params.outputAll) { //(searchData.symmetry == C1 && !searchData.supressOutput && searchData.catalysts.size() <= 4) {//TEMPORARY//if (isValid && params.outputAll) {
                ...
                allOutputsCategoryContainer.Add(searchData.startState, stateEvolution[sampleGenerationIndex], searchData.intermediateForms, CatContainerKey(diffState, searchData.matchStates[0].first, 0));
            ...
            if (eventualPeriod > 2) {
                ...
                if (AddOscillatorsToCollection(stateEvolution[sampleGenerationIndex], eventualPeriod)) {
                    ...
                    categoryContainerLock.lock();
                    oscillatorCategoryContainer.Add(searchData.startState, stateEvolution[sampleGenerationIndex], searchData.intermediateForms, CatContainerKey(diffState, searchData.matchStates[0].first, eventualPeriod));
                    categoryContainerLock.unlock();
                }
            }
                ...
                for (auto &matchPoint : newMatchPoints) {
                    ...
                    for (unsigned i = 0; i < params.partialCatsMustSurviveGens; i++) {
                        ...
                        for (unsigned catID = 0; catID < catData.size(); catID++) {
                            std::pair<bool, LifeState> filtering = CatalystCheckState(catData[catID][0], testState, catData[catID][1], catData[catID][2], i + 1 + std::get<0>(matchPoint));
                            if (!filtering.first) {
                                failure = true;
                            }
                            else if (filtering.second.IsEmpty()) {
                    ...
                    if (!failure) {
                        ...
                        categoryContainerLock.lock();
                        categoryContainer.Add(searchData.startState, stateEvolution[std::get<0>(matchPoint) - searchData.generation], searchData.intermediateForms, CatContainerKey(transformedState, searchData.signatureState, (int)std::get<0>(matchPoint)));
                        categoryContainerLock.unlock();
                        break;
                    }
                }
            ...
            for (unsigned generationIndex = params.minInteractionGeneration - std::min(searchData.generation, params.minInteractionGeneration); generationIndex < maxGenerationToTest; generationIndex++) {
                ...
                if (searchData.numCatalysts < searchData.maxCatalysts && (searchData.symmetry != params.symmetry || searchData.numCatalysts < params.maxCatalystsBeforeSymmetry) && (searchData.numCatalysts > 0 || generationIndex + searchData.generation < params.maxFirstCatalystInteractionGeneration)) {
                    ...
                    for (; catIndex < (unsigned)catalysts.size(); catIndex++) {
                        ...
                        if (
                            //TEMPORARY
                            //(!catalysts[catIndex].transparent && searchData.numCatalysts == 0) ||
                            //(catalysts[catIndex].sacrificial && searchData.numCatalysts == 0) ||
                        ...
                        if (!catalysts[catIndex].spaceship && !catalysts[catIndex].spark) {
                        ...
                        if (!catalysts[catIndex].spaceship && !catalysts[catIndex].spark) {
                            ...
                            LifeState stateRotated = catalysts[catIndex].locus;
                            ...
                            if (catalystPositions.state[x] != 0ULL) {
                                for (; y < 64; y++) {
                                    if (catalystPositions.GetCell(x, y) == 1) {
                                        ...
                                        if (!catalysts[catIndex].spaceship && !catalysts[catIndex].spark) {
                                        ...
                                        else if (catalysts[catIndex].spark) {
                                            //SPARK
                                            ...
                                            LifeState catStartState = catalysts[catIndex].sparkStart.GetSymChain(x, y, SymmetryChainFromEnum(searchData.symmetry));
                                            newEndCurrentState.Join(catCurrentState);
                                            LifeState evolvingStartState = newStartState;
                                            bool cantStartInteraction = false;
                                            for (unsigned stateGen = 0; stateGen < (generationIndex + searchData.generation); stateGen++) {
                                                if (stateGen == generationIndex + searchData.generation - catalysts[catIndex].sparkGens) {
                                                    if (!evolvingStartState.AreDisjoint(catStartState)) {
                                                        cantStartInteraction = true;
                                                        break;
                                                    }
                                                    evolvingStartState.Join(catStartState);
                                                }
                                                evolvingStartState.Step();
                                            }
                                            if (cantStartInteraction || evolvingStartState != newEndCurrentState) continue;

                                            //update catStateSymChains
                                            for (unsigned gen = 0; gen < catalysts[catIndex].sparkGens; gen++) {
                                                catStateSymChains.push_back(catStartState);
                                                catStartState.Step();
                                            }

                                            //this doesn't work with filters at all
                                            newCurrentState = newEndCurrentState;
                                        }
                                        else {
                                            LifeState newEndCurrentState = stateEvolution[generationIndex];
                                            LifeState catCurrentState = catalysts[catIndex].state.GetSymChain(x, y, SymmetryChainFromEnum(searchData.symmetry));
                                        ...
                                        newData.past1NeighborWithPeriod = past1NeighborWithPeriodEvolution[nextUseGenerationIndex];
                                        newData.past2NeighborsWithPeriod = past2NeighborsWithPeriodEvolution[nextUseGenerationIndex];
                                        ...
                                        newData.intermediateForms = searchData.intermediateForms;
                                        if (catalysts[catIndex].spark) {
                                            newData.intermediateForms.push_back(newData.currentState);
                                        }
                                        else {
                                            for (unsigned formId = 0; formId < newData.intermediateForms.size(); formId++) {
                                                newData.intermediateForms[formId].Join(catStateSymChains[0]);
                                            }
                                        }
                                        ...
                                        /*if (searchData.numCatalysts > 0 && searchData.generation > 0 && nextUseGenerationIndex > searchData.generation) {
                                            printLock.lock();
                                            printf("Data:\n");
                                            newData.startState.Print();
                                            newData.currentState.Print();
                                            for (unsigned gen = 0; gen < catalystPeriodLCM; gen++) {
                                                printf("Gen: %d\n", gen);
                                                newData.past1NeighborWithPeriod[gen].Print();
                                                newData.past2NeighborsWithPeriod[gen].Print();
                                                newData.past3NeighborsWithPeriod[gen].Print();
                                            }
                                            printLock.unlock();
                                        }*/
                                        ...
                                        if (!recoveryCheckingIDs.empty()) {
                                            ...
                                            while (extraGen < params.maxGeneration - (generationIndex + searchData.generation)) {
                                                ...
                                                for (unsigned catID : recoveryCheckingIDs) {
                                                    ...
                                                    //neutralRecovery = catID + 2 > newData.catalysts.size() && catalysts[thisCatIndex].transparent && newData.numTransparentCatalysts == 1 && positiveRecoveryThisGen && neutralRecoveryThisGen;
                                                    neutralRecovery = false;
                                                    //neutralRecovery = catID == 0 && catalysts[thisCatIndex].transparent && positiveRecoveryThisGen && neutralRecoveryThisGen;

                                                    if (neutralRecovery) positiveRecovery = false;

                                                    //TEMPORARY
                                                    //if (neutralRecovery && catID == newData.catalysts.size() - 1) neutralRecovery = false;
                                                    if (neutralRecovery) neutralRecovery = false;
                                ...
                                if (transformPositions.state[(baseX + 64) % 64] != 0ULL) {
                                    for (int baseY = sharedXSymmetry ? 0 : -(maxCoords.second / 2); baseY < (maxCoords.second + 1) / 2; baseY++) {
                                        if (transformPositions.GetCell(baseX, baseY) == 1) {
                                            ...
                                            newData.intermediateForms = searchData.intermediateForms;
                                            for (unsigned formId = 0; formId < newData.intermediateForms.size(); formId++) {
                                                newData.intermediateForms[formId] = newData.intermediateForms[formId].GetSymChain(x, y, generationChain);
                                                newData.intermediateForms[formId].Move(xOffset, yOffset);
                                            }

                                            for (unsigned gen = 0; gen < catalystPeriodLCM; gen++) {
                                                newData.past1NeighborWithPeriod.push_back(past1NeighborWithPeriodEvolution[nextUseGenerationIndex][gen].GetSymChain(x, y, generationChain));
                                                newData.past1NeighborWithPeriod[newData.past1NeighborWithPeriod.size() - 1].Move(xOffset, yOffset);
                                                newData.past2NeighborsWithPeriod.push_back(past2NeighborsWithPeriodEvolution[nextUseGenerationIndex][gen].GetSymChain(x, y, generationChain));
                                                newData.past2NeighborsWithPeriod[newData.past2NeighborsWithPeriod.size() - 1].Move(xOffset, yOffset);
                                                newData.past3NeighborsWithPeriod.push_back(past3NeighborsWithPeriodEvolution[nextUseGenerationIndex][gen].GetSymChain(x, y, generationChain));
                                                newData.past3NeighborsWithPeriod[newData.past3NeighborsWithPeriod.size() - 1].Move(xOffset, yOffset);
                                            }
                                            ...
                                            if (params.findPartials) {
                                                ...
                                                newData.matchStates = searchData.matchStates;
                                                newData.signatureState = searchData.signatureState.GetSymChain(x, y, generationChain);
                                                newData.signatureState.Move(xOffset, yOffset);
                                                if (searchData.numCatalysts == 0) {
                                                    for (unsigned matchGeneration = searchData.generation + 1; matchGeneration <= nextUseGenerationIndex; matchGeneration++) {
    ...
    std::pair<bool, LifeState> CatalystCheckState(const int &catIndex, const LifeState &state, const int &catX, const int &catY, const int &generation) {
        if (catalysts[catIndex].isBlinker) {
            ...
            return catalysts[catalysts[catIndex].evolutionIndices[generation % catalysts[catIndex].period]].CheckState(state, catX, catY);
        }
    }
